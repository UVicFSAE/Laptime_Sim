...
from lib.fit_equations import PolySurface, PolyCurve, EquationFits
...
class KinematicData:
    ...
    def _format_kinematic_data(self) -> None:
        ...
        for table in self.kinematic_tables:
            ...
            self.kinematic_tables[table].dropna(how='all', axis='columns', inplace=True)
    ...
    def _format_kinematic_table_headings(self):
        ...
        for table_key in kinematic_table_keys:
            if 'CAM' in table_key:
                ...
                new_key_prefix, new_key_suffix = table_key.split('CAM', maxsplit=1)
                ...
                self.kinematic_tables[new_table_key] = self.kinematic_tables[table_key]
                # Delete dataframe with the old key
                del self.kinematic_tables[table_key]
            else:
                # Make all letters lowercase
                self.kinematic_tables[f'{table_key}'.lower()] = self.kinematic_tables[table_key]
                # Delete dataframe with the old key
                del self.kinematic_tables[table_key]
    ...
    def _convert_kinematics_deg_to_rad(kinematic_table: df) -> None:
        for column_name in kinematic_table.columns:
            ...
            if column_name_type == str and '[deg]' in column_name:  # If column in degrees
                ...
                kinematic_table[column_name] = np.deg2rad(kinematic_table[column_name])
    ...
    def _create_left_side_kinematic_table(self):
        ...
        for table in self.kinematic_tables:
            self.left_side_kinematic_tables[table] = self.kinematic_tables[table].copy(deep=True)
            # Convert to ISO definitions of inclination and toe
            self._convert_kinematics_to_iso(self.left_side_kinematic_tables[table])

    def _create_right_side_kinematic_table(self):
        print('\t- Defining right_side_kinematic_tables...')
        self.right_side_kinematic_tables = {}
        for table in self.kinematic_tables:
            self.right_side_kinematic_tables[table] = self.kinematic_tables[table].copy(deep=True)
...
class KinematicEquations:
    ...
    def __init__(self,
        ...
        kinematic_fit_types = {'k_incln_f': kin_incln_f_sfit,  'k_incln_r': kin_incln_r_cfit,
                               'k_toe_f': kin_toe_f_sfit,      'k_toe_r': kin_toe_r_cfit,
                               'r_incln_f': roll_incln_f_cfit, 'r_incln_r': roll_incln_r_cfit}

        for kinematic in kinematic_fit_types.keys():
            fit = kinematic_fit_types[kinematic]
    ...
    def calculate_inclination_angles(self, bump_travel_mm, rack_travel_mm, roll_angle_rad):

        # Calculate front inclination angle
        front_kin_incln_rad = self.k_incln_f.calculate([bump_travel_mm, rack_travel_mm])
        ...
        rear_kin_incln_rad = self.k_incln_r.calculate([bump_travel_mm, rack_travel_mm])
    ...
    def fit_kinematics(self, kinematic_tables: dict[str: df], plot=False) -> None:
        ...
        for kinematic in vars(self):
            ...
            if type(kinematic_obj) == PolySurface:
                X, Y, x, y, z = self.prepare_kin_surface_data(kinematic_tables[kinematic])
                ...
                x, x_param, y, y_param = self.prepare_kin_curve_data(kinematic_tables[kinematic])
    ...
    @staticmethod
    def prepare_kin_curve_data(data: df) -> tuple[np.ndarray, str, np.ndarray, str]:
    ...
    def prepare_kin_surface_data(data: df) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Create a 2D mesh
        Y, X = np.meshgrid(steering_rack_data_mm, bump_travel_data_mm)  # Y = Rack Travel [mm], X = Bump [mm]
    ...
    @staticmethod
    def plot_kin_surface_fit(X: np.ndarray, Y: np.ndarray, z: np.ndarray, z_pred: np.ndarray, param: str = '') -> None:
    ...
    @staticmethod
    def plot_kin_curve_fit(x: np.ndarray, y: np.ndarray, y_pred: np.ndarray,
                           x_param: str = '', y_param: str = '') -> None:
...
if __name__ == "__main__":
    ...
    kinematics_left = KinematicEquations(kin_tables.left_side_kinematic_tables, plot_fit=True)
