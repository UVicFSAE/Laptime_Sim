...
class KinematicData:
    ...
    def _convert_bump_mm_to_m(self) -> None:
        kinematic_table_keys = list(self.kinematic_tables.keys())
        for table_name in kinematic_table_keys:
            if "k" in table_name:
                self.kinematic_tables[table_name].iloc[:, 0] /= 1000
                self.kinematic_tables[table_name].rename(
...
class KinematicEquations:
    ...
    @staticmethod
    def plot_kin_curve_fit(
        x: np.ndarray, y: np.ndarray, y_pred: np.ndarray, x_param: str = "", y_param: str = ""
    ) -> None:
